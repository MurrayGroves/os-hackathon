...
time1 = time.time()
df = pd.read_json("../new chargers/net.json")
print(f"Time to load JSON: {time.time()-time1}")
time1 = time.time()
df['co-ord'] = df.apply(lambda x: OSGB36toWGS84((x.x+4)*1000, (1218-x.y)*1000), axis=1)
print(f"Time to convert co-ords: {time.time()-time1}")
lat = []
lon = []
for i in df["co-ord"]:
    lat.append(i[0])
...
df = df.assign(lat=lat, lon=lon, kw2=kw2)
df.to_csv("../new chargers/heatmap_data.csv")
